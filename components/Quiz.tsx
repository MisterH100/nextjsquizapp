...
export const Quiz =()=>{
  ...
  const {setCurrCorrectQuizzes,setPoints,setCurrIncorrectQuizzes,setCurrQuiz,setComplete,complete,currQuiz,currCorrectQuizzes,currIncorrectQuizzes,timed,setTimed,start,updateData,authPlayer} = useQuizContext();
  ...
  const compareAns =(quiz:IQuiz,ans:string)=>{
    if(quiz.correctAnswer == ans){
      setCurrCorrectQuizzes([...currCorrectQuizzes,{...quiz, answered: true}]);
      setPoints((prev)=> prev + quiz.points);
      setProgress(100)
    }else{
      setCurrIncorrectQuizzes([...currIncorrectQuizzes,{...quiz, answered: true,answer:ans}]);
      setProgress(100)
    }
  }
  const nextQuiz = ()=>{
